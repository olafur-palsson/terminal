...
def main():
    ...
    if last_status == 'PASS' and fails:
        playsound('{current_folder}/fail.mp3')
        set_status('FAIL')
    elif last_status != 'PASS' and not fails:
        playsound('{current_folder}/pass.mp3')
        set_status('PASS')

def get_status():
    with open('{current_folder}/status', 'r') as status_file:
        status = next(line for line in status_file)
        return status

def set_status(status):
    with open('{current_folder}/status', 'w') as status_file:
